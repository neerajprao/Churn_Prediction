...
def load_models():
    ...
    # Load XGBoost model
    try:
        # Bypassed on Mac to prevent segmentation fault
        # xgb_model = XGBClassifier()
        # xgb_model.load_model(os.path.join(BASE_DIR, 'models', 'xgboost_model.json'))
    ...
    try:
        # Bypassed on Mac to prevent segmentation fault
        # lgbm_model = Booster(model_file=os.path.join(BASE_DIR, 'models', 'lightgbm_model.txt'))
    ...
    try:
        ...
        catboost_model.load_model(os.path.join(BASE_DIR, 'models', 'catboost_model.cbm'))
    except Exception as e:
    ...
    try:
        ...
        node_model.load_state_dict(torch.load(os.path.join(BASE_DIR, 'models', 'node_model.pt')))
        node_model.eval()
    ...
    try:
        ...
        saint_model.load_state_dict(torch.load(os.path.join(BASE_DIR, 'models', 'saint_model.pt'), strict=False))
        saint_model.eval()
    ...
    try:
        ...
        tabtransformer_model.load_state_dict(torch.load(os.path.join(BASE_DIR, 'models', 'tabtransformer_model.pt'), strict=False))
    ...
    try:
        ...
        try:
            ...
            tabnet_model.load_state_dict(torch.load(os.path.join(BASE_DIR, 'models', 'tabnet_model.pt'), strict=False))
            tabnet_model.eval()
            ...
            saved_model = torch.load(os.path.join(BASE_DIR, 'models', 'tabnet_model.pt'))
